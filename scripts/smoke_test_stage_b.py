...
# 设置模型路径必须在导入 config 之前（config 在导入时读取环境变量）
SAKURA_GGUF = str(ROOT / "models" / "sakura" / "sakura-14b-qwen2beta-v0.9.2-q4km.gguf")
os.environ["JPZH_LLM_MODEL"] = SAKURA_GGUF

from jpzh_subtitle.config import LLM_MODEL_PATH, MODELS_DIR  # noqa: E402
...
def main() -> int:
    ...
    if not Path(LLM_MODEL_PATH).is_file():
        print(f"Sakura 模型未就绪: {LLM_MODEL_PATH}", flush=True); return 1

    print(f"运行阶段 B: {SAMPLE.name} -> 中文 srt", flush=True)
    print(f"  ASR 模型: {ASR_MODEL}", flush=True)
    print(f"  LLM 模型: {LLM_MODEL_PATH}", flush=True)
